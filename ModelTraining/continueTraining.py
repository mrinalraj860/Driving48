...
train_loader = DataLoader(remapped_train_dataset, batch_size=8, shuffle=True, collate_fn=motion_collate_fn)
...
model = PointTemporalGRU(num_classes=47).to(DEVICE)
...
for epoch_offset in range(1, 21):
    ...
    for inputs, labels, _ in tqdm(train_loader, desc=f"Training Epoch {epoch}"):
    ...
    with torch.no_grad():
        for inputs, labels, _ in tqdm(test_loader, desc=f"Testing Epoch {epoch}"):
            inputs, labels = inputs.to(DEVICE).float(), labels.to(DEVICE)
            outputs = model(inputs)
